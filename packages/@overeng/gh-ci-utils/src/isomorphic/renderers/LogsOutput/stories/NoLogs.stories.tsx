/** No completed jobs yet — logs not available. */
...
import { createNoLogsTimeline, loadingState, noLogsState } from './_fixtures.ts'
  ...
  render: function Render(args) {
    return (
      <TuiStoryPreview
        ...
        command={`gh-ci-utils logs${args.target ? ` ${args.target}` : ''}${args.interactive ? ' --watch' : ''}`}
        {...createInteractiveProps({
          args,
          staticState: noLogsState(),
          idleState: loadingState(),
          createTimeline: () => createNoLogsTimeline(),
        })}
      />
    )
  },
