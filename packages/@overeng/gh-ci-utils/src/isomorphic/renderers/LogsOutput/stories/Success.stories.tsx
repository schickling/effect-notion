/** Successful lint job with devenv task runner output. */
...
import { createSuccessTimeline, loadingState, successState } from './_fixtures.ts'
...
export default {
  title: 'gh-ci-utils/Logs/Success',
  component: LogsView,
  argTypes: {
    ...commonArgTypes,
    target: {
  ...
  render: function Render(args) {
    return (
      <TuiStoryPreview
        ...
        command={`gh-ci-utils logs${args.target ? ` ${args.target}` : ''} --job lint${args.interactive ? ' --watch' : ''}`}
        {...createInteractiveProps({
          args,
          staticState: successState(),
          idleState: loadingState(),
          createTimeline: () => createSuccessTimeline(),
        })}
      />
    )
  },
