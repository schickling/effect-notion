...
/** One entry of a collection artifact: a test name and its package-relative POSIX source file. */
...
const isNormalizedPackageRelativePath = (value: string): boolean =>
  value.length > 0 &&
...
export const decodeTaskRegistration = (value: unknown): TestTaskRegistration | undefined => {
  ...
  if (
    ...
    isNormalizedPackageRelativePath(packagePath) === false ||
    taskName.length === 0 ||
    collectionTarget.length === 0
  ) {
    return undefined
  }
  return { collectionTarget, packageName, packagePath, taskName }
}
...
const decodeCollectionEntries = (value: unknown): readonly CollectionEntry[] | undefined => {
  ...
  const decoded = value.map((entry: unknown) => {
    ...
    return typeof name === 'string' &&
      typeof file === 'string' &&
      isNormalizedPackageRelativePath(file) === true
...
const evaluateTask = async ({
  ...
  return files.map((file): FileResult => {
    const packageRelativeFile = posix.relative(registration.packagePath, file)
    if (isNormalizedPackageRelativePath(packageRelativeFile) === false) {
      return {
    ...
    const collectedTests = entries.filter((entry) => entry.file === packageRelativeFile).length
    if (collectedTests === 0) {
      return {
        collectedTests,
  ...
  const registered = (
    await Promise.all(
      [
        ...Map.groupBy(
          registeredFiles,
          ({ registration }) => registration.collectionTarget,
        ).values(),
      ].map((group) =>
        evaluateTask({
