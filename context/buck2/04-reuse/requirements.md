...
- **REUSE-R03 Capacity envelope:** The surface holds warm no-op ≤ 5 s and fresh
  ...
  `buck-out`/output/scratch disk, retained editor-snapshot disk and generation
  count, staging/action p95, and marginal time/disk/action-count envelope.
  Staged prerequisite work measures this in its named candidate namespace.
  Raising timeout or disk alone is insufficient; regression blocks widening.
...
- **REUSE-R07 Output and local-disk economics:** Uploads contain shared package
  ...
  entries with platform-selected edges own one per configured variant (a
  lockfile-derived set, ten in the current complete lock per BUCK-R08).
