...
per selected platform-edge variant for exactly nine entries; archive/extract
bytes remain shared. Dependency edges and importer/scratch views remain
metadata-only, while workspace/package views own only their small boundary.
Per-consumer dependency closure materialization is rejected even where CoW is
unavailable. The editor availability snapshot intentionally owns copied bytes.
...
- BUCK-R08 and decision 0030 require one package copy per normalized identity
  (per selected platform-edge variant for nine entries), shared archive bytes,
  metadata-only dependency/importer views, and no per-consumer closure copy,
  independent of filesystem CoW.
...
identity (per selected platform-edge variant for nine entries), metadata-only
dependency/importer views, bounded workspace/package boundaries, and the
byte-owned editor snapshot exception. Implementation progress alone does not
close this delta.
