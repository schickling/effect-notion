...
let
  ...
  platformKeys = map (
    platform: platformKey (builtins.removeAttrs platform [ "system" ])
  ) targets.platforms;
  ...
  checkEntry =
    ...
    let
      ...
      tagMatch = builtins.match ("buck2-native-product-v1-${name}-${platform.os}-${platform.architecture}-${platform.abi}-([0-9a-f]{64})") release.tag;
      ...
      checks = [
        ...
        (ensure (
          descriptor.semanticProvenance.target == targetByName.${name}
        ) "${key} target does not match the release declaration")
        (ensure (builtins.elem platform (
          map (value: builtins.removeAttrs value [ "system" ]) targets.platforms
        ...
        (ensure (
          release.name
          == "${payloadDigest}-${name}-${platform.os}-${platform.architecture}-${platform.abi}.tar"
        ) "${key} release asset name does not match its tag")
        (ensure (
          release.url == "${repositoryReleaseBase}/${release.tag}/${release.name}"
        ) "${key} release URL does not match its immutable tag and asset")
        (ensure (
          release.hash == descriptor.payload.digest.sri
  ...
  currentPlatform = lib.findFirst (
    platform: platform.system == pkgs.stdenv.hostPlatform.system
  ) null targets.platforms;
  productsForPlatform =
    platform:
    let
  ...
  products = if currentPlatform == null then { } else productsBySystem.${currentPlatform.system};
  topChecks = [
    (ensure (
    ...
    (ensure (
      manifest.schema == "effect-utils/buck2-native-release-products/v1"
    ) "unsupported manifest schema")
    (ensure (
      targets.schema == "effect-utils/buck2-native-release-targets/v1"
    ) "unsupported target schema")
    (ensure (builtins.all (
      platform:
      builtins.attrNames platform == [
        "abi"
        "architecture"
        "os"
        "system"
    ...
    (ensure (
      builtins.length platformKeys == builtins.length (lib.unique platformKeys)
    ) "target platforms contain duplicate native tuples")
    (ensure (
      builtins.length systems == builtins.length (lib.unique systems)
    ...
    (ensure (
      builtins.length productTargets == builtins.length (lib.unique productTargets)
    ) "target products contain duplicate labels")
    (ensure (
      builtins.length actualKeys == builtins.length expectedKeys
    ) "manifest matrix has the wrong size")
    (ensure (
      builtins.length actualKeys == builtins.length (lib.unique actualKeys)
    ) "manifest matrix contains duplicates")
    (ensure (
      lib.sort builtins.lessThan actualKeys == lib.sort builtins.lessThan expectedKeys
