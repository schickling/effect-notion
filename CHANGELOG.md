...
- **@overeng/tui-react**: `runTuiMain` derives the process exit code from the fiber `Exit` alone. An interrupt (Ctrl-C) stays in the error channel — only its stack is suppressed — and the `teardown` delegates to `Runtime.defaultTeardown`: interrupts-only → **130** (the shell convention), any other failure → its `Runtime.errorExitCode` marker or 1, success → 0 (or the app-level `process.exitCode` a `createTuiApp` `exitCode` mapper assigned on unmount). Previously the interrupt branch swallowed the interrupt into a `Success` and smuggled 130 through the global `process.exitCode`, which the teardown read back — so any stale ambient value (an earlier phase, or a previous test in the same process, where Bun ignores `process.exitCode = undefined`) turned a clean success into a non-zero exit. The shutdown-flush + exit-code contract is captured in `docs/spec.md` (Lifecycle & Cleanup) and `docs/.decisions/0001-shutdown-flush-contract.md`, and locked by a fast deterministic regression test (`test/integration/cli-shutdown-flush.test.ts`): natural exit awaits + lands the flush and is fast, a signal exits 130 without touching `process.exitCode`, success exits 0 — plus a real-subprocess failure case (`test/integration/runResult-stdout-contract.test.ts`) proving exit 1 with the formatted cause on stderr. (#771)
